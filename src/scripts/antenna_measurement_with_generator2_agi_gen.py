...
# OJOOO!!!! ESTE SCRIPT SE HIZO PARA USAR AGILENT, HAY QUE CONFIGURAR EL GENERADOR MANUALMENTE
...
WAIT_TIME_AFTER_MEASUREMENT = 0.25   # in seconds
LOWEST_EL_ANGLE = 30    # Angle between zenith and lowest el point to measure

# sweep = [1200, 1300, 1400, 1500, 1600, 1700, 1800]
# sweep = [2140, 2157, 2170]
# sweep = [725, 825, 900, 1750, 1850, 2150, 2450, 2550]
# sweep = [850, 1850, 2650]
sweep = [3500]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-rbw','--res_bw', type = float, default = 75, help ='resolution bandwidth in kHz. 75 kHz by default')
    parser.add_argument('-vbw','--video_bw', type = float, default = 30, help ='video bandwidth in kHz. 30 kHz by default')
    parser.add_argument('-swt','--sweep_time', type = float, default = 10, help ='sweep time in ms. 10 ms by default')
    parser.add_argument('-cp','--curve_points', type = int, default = 1000, help = 'number of pointer per curve. 1000 by default')
    ...
    parser.add_argument('-wait','--sa_wait_time', type = int, default = 1, help = 'Spectrum analizer wait time, in seconds, before taking the curve')
    ...
    parser.add_argument('--gen_power', type = float, default = 20, help = 'Generator power in dBm')
    ...
    try:
        MIN_FREQ_default = args.minimum_freq_ghz * 1e9
        #print('Frecuencia en python:',MIN_FREQ_default)
    except:
        MIN_FREQ_default = 1e9 # 1 GHz

    # Defining maximum freq span
    try:
        MAX_FREQ_default = args.maximum_freq_ghz * 1e9
    except:
        MAX_FREQ_default = 1.1e9 # 1.1 GHz
    ...
    angles = np.zeros(STEPS_PER_LOOP)
    spectrum = np.zeros((len(sweep), STEPS_PER_LOOP))
    spectrum2 = np.zeros((len(sweep), STEPS_PER_LOOP))
    ...
    for i in range(STEPS_PER_LOOP):
        ...
        for i in range(len(sweep)):
            ...
            # time.sleep(0.1)
            #siglent.configure_frequency(sweep_frequency * 1e6, 1e6)#2   e6 cambiazo
            # time.sleep(0.1)
            result = point_and_measure(target_angle, measure_wait_time, siglent_use)
            spectrum[i, contador] = np.max(result[2])
            spectrum2[i, contador] = result[2][target_point]
            if(real_time_plot):

                target_point = int(CURVE_POINTS / 2)
                angles_plot = np.array(np.arange(0, angles[contador], DEGREE_STEP)).astype(float)
                power_plot = np.array(spectrum[i, 0:contador]).astype(float)
                freq_plot = sweep_frequency
                ax.cla()
    ...
    np.savetxt(name + '.csv', spectrum, delimiter = ",", fmt = '%1.4f')
    np.savetxt(name + '2.csv', spectrum2, delimiter = ",", fmt = '%1.4f')
    np.savetxt(name + 'angles.csv', angles, delimiter = ",", fmt = '%1.4f')
    np.savetxt(name + 'frequencies.csv', sweep, delimiter = ",", fmt = '%1.4f')

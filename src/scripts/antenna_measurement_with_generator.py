...
sweep=[1400,1500,1600,1700,1800]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gen_power', help = 'Generator power in dBm')
    ...
    for i in range(len(sweep)):
        try :
            ...
            file = open(str(i)+name,'rb')

            file.close()
        except FileNotFoundError:
            #if file does not exist initialize it
            file = open(str(i)+name,'wb')
    ...
    if (siglent_use):
        ...
        visaname = 'TCPIP0::10.17.90.140::inst0::INSTR'
    ...
    measured_spectra = []
    for i in range(len(sweep)):
        measured_spectra.append(lista_vacia)

    target_angle = 0
    while abs(target_angle) <= 360 + extra_angle:
        for i in range(len(sweep)):
            file = open(str(i)+name,'ab')
            ...
            measured_spectra[i].append(result)
            file.close()
            if(real_time_plot):
                #ipdb.set_trace()
                target_point = int(CURVE_POINTS/2)
                aux = np.array(measured_spectra[i])

...
# sweep_freq = [500, 3000]    #MHz

sweep_freq = [3400, 3600]    #MHz

points = sweep_freq[1] - sweep_freq[0] + 1
...
def point_and_measure(angle, measure_wait_time, siglent_use):
    ...
    print(f'Tardó {time.time()-t} segundos')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-wait','--vna_wait_time', type = int, default = 1, help = 'vna wait time, in seconds, before taking the curve')
    parser.add_argument('--siglent_measure', action = 'store_true',help = "write this if you want to use the vna", default=True)
    parser.add_argument('--no_siglent_measure', dest = 'vna_measure', action = 'store_false', help = "write this if you DON'T want to use the vna")
    ...
    try:
        MIN_FREQ = args.minimum_freq_ghz * 1e9
    except:
        MIN_FREQ = sweep_freq[0] * 1e6

    # Defining maximum freq span
    try:
        MAX_FREQ = args.maximum_freq_ghz * 1e9
    except:
        MAX_FREQ = sweep_freq[1] * 1e6
    ...
    measure_wait_time = args.vna_wait_time

    print("Using " + str(measure_wait_time) + " seconds wait time")
    time.sleep(1)

    # Defining if we are going to use the vna to measure
    ...
    #     file = open(name,'wb')
    #     config_dict = {'STEPS_PER_LOOP': STEPS_PER_LOOP,
    ...
    if (siglent_use):
        # Initializing VNA ethernet communication
        visaname = 'TCPIP0::10.17.90.141::inst0::INSTR'
        siglent = sva1075x.sva1075x(visaname)

        # Configuring VNA Parameters
        pts = points
        # freq = [500 * 1e6, 3000 * 1e6]
        freq = [3400 * 1e6, 3600 * 1e6]

        siglent.configure_vna(freq, pts)
    ...
    np.savetxt('freq_axis_' + name, meas[2][0], delimiter = ",", fmt = '%1.4f')
